...
const transformarRotasParaDto = (rotas: any[]): ComandoDto[] => {
  return rotas.map(rota => {
    if (rota.action === 'para frente') {
      return {
        acao: 'frente',
        valor: Number(rota.value) // Converte string '50' para número 50
      };
    }
    ...
    // Filtra qualquer item inesperado
    return null;
  }).filter(Boolean) as ComandoDto[]; // Remove os nulos
};
...
export default function Home() {
  ...
  const handleIniciar = async () => {
    ...
    if (rotasExemplo.length === 0) {
        alert("Adicione pelo menos um comando ao trajeto!");
        return;
    }

    // 1. "Traduzir" o estado para o DTO
    ...
    try {
        // 2. Chamar o backend Nest.js (que por sua vez chama o ESP32)
        // Certifique-se que seu backend Nest.js está rodando em http://localhost:3001
        const response = await axios.post(
            'http://localhost:3001/carrinho/executar', 
            payload
        );
        
        console.log("Resposta do backend:", response.data);
        alert("Trajeto enviado para o carrinho com sucesso!");
        // Opcional: limpar as rotas após o envio
        // setRotasExemplo([]);

    } catch (error) {
        console.error("Erro ao enviar trajeto:", error);
        alert("Falha ao enviar trajeto para o backend.");
    }
 };
